...
export class IamGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const principal = await this.principal(request, key, isPublic);
      ...
      if (context.getType() === 'http') this.checkCsrf(request);
  ...
  private async principal(
    ...
    if (isPublic && !headers.has('authorization') && !headers.has('cookie')) return null;
    try {
      return await this.options.iam.authenticate(credentialOf(request));
    } catch (error) {
      if (isPublic && isAuthenticationError(error)) return null;
      throw error;
    }
  }

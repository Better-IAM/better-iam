...
- `useMyApps({ tenantId })` loads the signed-in person's app launcher (`applications.mine`): `apps` they can open now (most recently used first, each with `via`: `everyone`, `direct` or `group`) and `requestable` apps they may ask for through an access package (`requestPackageId`). `launch(appId)` records the launch and resolves to the app's sign-in URL; open it yourself. See [application catalog](../../docs/applications.md).

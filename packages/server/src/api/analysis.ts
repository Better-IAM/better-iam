...
function unrestrictedAdmin(document: PolicyDocument | undefined): boolean {
  return (
...
export function createAnalysisApi(ctx: ServerContext) {
  ...
  return {
    /** Runs every check. `dormantDays` (default 90) sets when an unused account holding access is reported. */
    findings: (
      ...
      operation(
        ...
        async ({ tx, tenant }) => {
          ...
          const members = (await tx.find<GroupMember>('groupMembers', { tenantId })).filter(
          ...
          for (const identity of identities) {
            ...
            if (admin && identity.kind === 'user' && !tenantRequiresMfa) {
              const factor =
                (await tx.get<StoredRecord & { enabled?: boolean }>('authMfa', identity.id))
                  ?.enabled === true ||
          ...
          if (agentIds.size) {
            ...
            for (const event of await findOrdered<AuditEvent>(
              ...
              if (agentId && agentIds.has(agentId))
                denials.set(agentId, (denials.get(agentId) ?? 0) + 1);
            }
          ...
          if (teams.length) {
            ...
            const teamMembers = (
              await tx.find<TeamMember>(teamCollections.members, { tenantId })
          ...
          const suppressions = new Map(
            (await tx.find<Suppression>('analysisSuppressions', { tenantId })).map((row) => [
              row.findingId,
              row,
            ]),
          );
    ...
    /** Hides one finding from future results, with a reason kept for reviewers. Requires iam:analysis:update. */
    suppress: (
      credential: CredentialInput,
      ...
      operation(
        ...
        async ({ tx, principal }) => {
          ...
          const record: Suppression = {
            ...
            reason: text(input.reason, 'reason', 500).trim(),

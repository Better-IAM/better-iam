...
Policies can ask which device a request comes from, and whether your MDM or EDR manages that device and finds it in
good shape. A browser or agent keeps a private key and enrolls the public half once. After that, every request can
carry a short-lived signed proof, bound to the session it travels with. The server checks the proof against the
enrolled key and gives policies five keys: `request.deviceAssurance`, `request.deviceManaged`,
...
The feature is always on, with nothing to configure. It is the `devices` API group, and the browser and Node helper is
`@better-iam/client/device` (also `better-iam/client/device`).
...
| `registered`              | proves an active device, which no active integration manages                     | false           | false             |
...
Better IAM also has remembered devices: the "Remember this device" box at sign-in. The two features never mix.
...
A registered device never satisfies MFA, and a remembered browser never counts as a registered device. Removing one
leaves the other alone. For example, the threat response `forget-devices` forgets remembered devices and does not
touch registered ones.
...
Decisions verify the proof only when a policy document they read names `request.device`, and they verify it at most
once per request (a batch of `authorizeMany` checks shares one verification). Decisions never write anything.
`devices.check` records when a device and key were last seen, at most once a minute. Simulated principals never have a
device.
...
a "view as" session (the device must be theirs or shared). The SSH certificate sweep has no request, so it judges
device conditions by the device the issuing request proved, as that device stands now. A session acting in another
tenant than the device's (a cross-tenant role session) gets `registered` at most: that tenant neither manages the device
nor set the requirements it was judged by, so `request.deviceManaged` and `request.deviceCompliant` are false there.
...
A proof is not a per-request signature. It does not cover the method, URL or body, and it can be reused with the same
session for up to five minutes (the client helper reuses each proof for four). What it proves is that the holder of the
session could sign with the device's key in the last five minutes.
...
their own tenant, never while impersonating. Service accounts and agents cannot enroll devices.
...
The public key is an EC P-256 or Ed25519 JWK with public members only. A key with private members is refused. The key's
thumbprint becomes `keyId`. A key that is already enrolled anywhere is refused with `CONFLICT`. The new device is
unmanaged and owned by the caller. A person may have 20 devices that are not retired, a device may hold 20 keys, and
enrollment is rate limited to 30 attempts per person per rate-limit window.

Enrolling proves possession of the key: the request must carry a device proof signed with the key being enrolled, for
the enrolling session (`x-better-iam-device`, which the client helper's `headers()` adds once the session id is known),
or it is refused with `INVALID_INPUT`. Otherwise anyone who saw a public key (one proof is enough to recover it) could
enroll it first and lock its holder out. Enrolling and `retireMine` also need a recent sign-in
(`RECENT_AUTH_REQUIRED`), so a stolen session cannot bring a key of its own or take the person's devices away.
...
and only until it expires. When it names an owner, or its device has an owner, only that person can use it. The device
must still be `active`. A code without `deviceId` creates a new device at enrollment, owned by the code's person, or
shared when the code names nobody. A shared device may be presented by any identity of the tenant.

Only a hash of each code is stored. The code is shown once, in the answer to `createEnrollment`. Administrators see the
codes' status with `devices.listEnrollments` and withdraw one with `devices.revokeEnrollment`. A tenant can hold 1000
pending codes. Expired codes are deleted by the retention sweep (`iam.sweepExpired()`).
...
The key moves onto the managed record. If the key came from the same person's self-enrolled record of the machine, and
that record has no keys left, it is retired. Only a key already enrolled in the tenant can move, and only from an
active device of the owner the report names: that person's self-enrolled device or another record of the same
...
## Using `@better-iam/client/device`

The helper creates and keeps an ECDSA P-256 key pair and signs proofs with WebCrypto. It has no dependencies and runs
in browsers in a secure context (HTTPS or localhost) and in Node 22 or later.
...
`headers(sessionId)` resolves to `{ 'x-better-iam-device': proof }`, or to no headers when the session id is unknown or
no proof could be made. It never fails the request that asked for them; failures go to the `onError` option instead.
`now` corrects a clock that drifts more than five minutes: pass one based on `limits.now` from `auth.getSession()`.
...
// The person names the device and confirms the platform (guessed from navigator.userAgentData, for example).
export async function registerThisBrowser(
  tenantId: string,
  name: string,
...
`createDeviceProver({ scope: identityId })`: each scope keeps its own key pair in the default store.
...
agent should keep its key, or every start would enroll a new device. Supply your own `DeviceKeyStore`, for example one
backed by the operating system's keychain, or this file-based one:
...
A key in a file is only as safe as the file: anyone who can read it can sign as the device.
...
- the in-process clients of `@better-iam/next` and of the Express, Hono, Fastify and SvelteKit integrations, which
...
1. Create the integration. `vendor` is `intune`, `jamf`, `kandji`, `workspace-one`, `google-endpoint`, `crowdstrike`,
   `sentinelone` or `custom`. A tenant can have 20 integrations.
2. Give the connector its own service account, allowed only `iam:devices:report` on this integration.
3. Issue the service account an API key scoped to that action, and hand the key to the connector.
...
const integration = await iam.api.devices.createIntegration(admin, {
  ...
  name: 'Intune',
  vendor: 'intune',
  trustVendorCompliance: true, // the default: count Intune's own verdict too
});

const reporter = await iam.api.serviceAccounts.create(admin, {
  tenantId,
...
and off. Its devices stop counting as managed while it is disabled, and its reports are refused
(`INVALID_TRANSITION`). `devices.deleteIntegration` refuses while the integration manages devices, unless `detach: true`
makes them unmanaged.
...
| Field                             | Meaning                                                                                                           |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `externalId`                      | Required. The device's id at the vendor: printable ASCII, at most 200 characters, once per report                 |
| `platform`                        | Required. `windows`, `macos`, `linux`, `ios`, `android`, `chromeos` or `other`                                    |
| `name`, `serialNumber`, `model`   | Optional text, at most 128 characters each. The name falls back to the model, serial number or id                 |
| `osVersion`                       | Up to 64 characters. The leading dotted number is kept (`14.5 (23F79)` reads as `14.5`); anything else is unknown |
| `ownerEmail` or `ownerIdentityId` | The person the device belongs to (a person of the tenant). An unknown owner leaves the device without one         |
| `keyThumbprint`                   | The thumbprint of a key already enrolled on the machine: the key moves onto this record                           |
| `posture`                         | Booleans `compliant` (the vendor's verdict), `encrypted`, `firewall`, `screenLock`, `edrHealthy`, `jailbroken`    |
| `checkedInAt`                     | When the device last checked in with the vendor, in epoch milliseconds (default now)                              |
...
- A report older than the device's last check-in is ignored. A repeated check-in that changes nothing is written at most
  once a minute.
...
A connector for Microsoft Intune, for example, maps Microsoft Graph `managedDevice` objects:
...
function fromIntune(device: ManagedDevice): DeviceReport {
  ...
  return {
    ...
    posture: {
      ...
      jailbroken:
        device.jailBroken === 'True' ? true : device.jailBroken === 'False' ? false : undefined,
    },
...
- The device is not `active`: `lost` or `retired`.
...
numbers. With only the defaults, any fresh managed device that is not jailbroken and not failed by its vendor is
compliant, so set the requirements you mean to enforce. Compliance is worked out at decision time, so a settings change,
a disabled integration, or a device going stale takes effect on the next request.

## Policies

Name the keys in any policy document a decision reads: role documents, policies of people and groups, and ceilings.

| Key                       | Type       | Present                                                                                    |
| ------------------------- | ---------- | ------------------------------------------------------------------------------------------ |
| `request.deviceAssurance` | identifier | Whenever a document names `request.device`: `none`, `registered`, `managed` or `compliant` |
...
| `request.deviceId`        | identifier | Only when a proof verified                                                                 |
| `request.devicePlatform`  | identifier | Only when a proof verified                                                                 |

Only people enroll devices, so rules that deny without a device should leave machines out with `principal.kind`
(`user` for people, `service` for service accounts, `agent` for agents). In a delegated session `principal.kind` is the
person's, so an agent acting for a person meets the person's device rules; test `principal.delegated` to treat it
differently.
...
- `request.deviceId` and `request.devicePlatform` are absent without a verified device, and a missing key never
  satisfies an operator. Prefer allow statements with them. A deny that tests them never applies to requests without a
  verified device: say so with `Exists: true`, as `NoMobileDownloads` does, or cover those requests with a second deny
  whose only condition is `Exists: false`. Policy lint suggests both.
- Try rules first. `policies.test` fills `request.deviceAssurance` with `none` and the two booleans with false unless the
  context passes them. Simulations (`policies.simulate`, `policies.whoCan`, `impact.preview`) evaluate people without
  a device, so they show what each person can do when they come without one.
- Root administrators use the platform override, so device rules do not constrain them.

## Administration

- `iam:devices:read`: `list` on `iam/devices`, `get` on `iam/devices/{id}`, `getSettings` on `iam/devices/settings`,
  and `listIntegrations` on `iam/devices/integrations`.
- `iam:devices:manage`: `update`, `retire`, `delete` and `removeKey` on `iam/devices/{id}`; `createEnrollment`,
  `listEnrollments` and `revokeEnrollment` on `iam/devices/enrollments`; `configure` on `iam/devices/settings`;
  `createIntegration` on `iam/devices/integrations`; `updateIntegration` and `deleteIntegration` on
  `iam/devices/integrations/{id}`. All of them except `listEnrollments` and `revokeEnrollment` need a recent sign-in.
- `iam:devices:report`: `report` on `iam/devices/integrations/{id}`.
- No permission: `enroll`, `mine` and `retireMine` from a person's own session, and `check` from any session of the
  tenant.

- `devices.list` filters by owner, status, platform, `managed`, `compliant` and a `query` over names, serial numbers,
  models, vendor ids and owners. Each device comes with its compliance, the assurance a proof from it would get now, and
  its number of keys.
...
keys the person enrolled, and codes issued for them are withdrawn. Every change is audited as a `device:*` event (see
[Events](events.md#device-posture-events)); subscribe a webhook to `device:compliance-change` to hear when an
integration's report takes a device out of compliance.

## Limitations

- **No hardware attestation.** A registered device proves possession of a key enrolled from a browser profile or agent,
  not that the hardware is genuine or that the key lives in a TPM or secure enclave. Anyone who copies the browser
  profile or the agent's key store can sign as the device. Script running in the page (for example after a cross-site
  scripting flaw) can make proofs while it runs, even though it cannot read the key.
- **Posture is only as good as the integration.** Better IAM does not inspect devices. It believes whatever the holder
  ...
  posture.
- **Proofs are short-lived bearer proofs.** With the same session they can be reused for five minutes, and they do not
  cover the method, URL or body.
...
- **The client helper signs with ES256 only.** The server also accepts Ed25519 keys (`EdDSA`) that other agents enroll.

...
| Threat response: sessions ended, identity contained | `threat:revoke-sessions`, `threat:contain` | CAEP `session-revoked`, RISC `account-disabled` |
...
events are pushed. To receive events from other identity providers, see the
...
<Cards>
  <Card title="Shared Signals receiver" href="/docs/federation/shared-signals-receiver">
    The other direction: take in CAEP and RISC events from upstream identity providers.
  </Card>

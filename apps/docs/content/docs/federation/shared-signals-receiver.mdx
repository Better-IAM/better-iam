...
one record), and every change also needs a <Term id="recent-authentication">recent sign-in</Term>. Your team only
schedules the poll job, and only when poll sources exist.
...
<TypeTable
  type={{
    ...
    actions: {
      ...
      description: 'The action per event type. Types not listed are recorded only. Any event type except the two control events may end sessions.',
    },
    requireTyp: {
      type: 'boolean',
...
| Status | `err` | Meaning |
| --- | --- | --- |
| 202 | (empty body) | The event is committed. A SET whose issuer and `jti` arrived before also gets 202, and nothing is done again. |
| 400 | `invalid_request`, `invalid_key`, `invalid_issuer`, `invalid_audience` | The SET was refused (see [Verification](#verification)); `description` says why. The same SET gets the same answer again. |
...
| 404 | `invalid_request` | No active push source has that ID: it is unknown, deleted, disabled, a poll source, or its organization is suspended. |
...
| 429 | `invalid_request` | The source sent more than 3000 events in the rate-limit window. `Retry-After` says when to try again. |
| 503 | `invalid_key`, `invalid_request` | The source's keys could not be obtained (`Retry-After: 30`) or storage was busy (`Retry-After: 1`). The provider should send it again. |
...
A run returns `{ sources, received, acknowledged, errors }`: new events, acknowledgements the provider accepted, and
failed requests plus events left for redelivery. A failed request is noted as the source's `lastError` (such as "The
poll endpoint answered 500"), and pending acknowledgements wait for the next attempt. There is no long polling, so the
job's interval is the delay. `signals.poll({ tenantId, sourceId })` polls one source now, for a "Check now" button.
...
in a `tenant_id` claim. Set `tenantClaim` to the upstream organization's ID, so the source accepts only that
...
<Callout title="Schedule the poll job">
  ...
  one source). It needs no credential, and what it records is audited by the actor `signal:{sourceId}`. Overlapping runs are harmless: an
  event delivered twice is recognized as a duplicate and acknowledged again. Deployments without poll sources can
  skip the job, since pushes need nothing scheduled.
</Callout>

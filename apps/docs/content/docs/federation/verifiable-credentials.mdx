...
`deleteType` also removes the type's pending offers. It is refused with `RESOURCE_IN_USE` while credentials of the
type are unexpired and not revoked (suspended ones count): disable the type, or revoke them first.
...
- `retireKey` removes a previous key from the published keys, after which its credentials no longer verify. It is
  refused for the active key (`INVALID_TRANSITION`), and with `RESOURCE_IN_USE` while unexpired, unrevoked credentials
  signed by the key exist, unless `force: true`, which revokes them (reason `key-retired`).

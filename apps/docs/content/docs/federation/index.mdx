...
import { ArrowDownToLine, ArrowUpFromLine, Bot, Building, IdCard, KeyRound, ListTree, LogIn, Radar, RadioTower, Server, ShieldCheck } from '@/lib/icons';
...
| Hear from a customer's IdP when a session is revoked, credentials leak, or an account is disabled | Shared Signals (CAEP, RISC) receiver | `signals` API group, `iam.signals.poll()` | [Shared Signals receiver](/docs/federation/shared-signals-receiver) |
| Give people signed credentials (an employee badge) they keep in a digital wallet | OpenID4VCI, SD-JWT VC, Token Status List | `verifiableCredentials` API group | [Verifiable credentials](/docs/federation/verifiable-credentials) |
| Let VPNs, NAS devices and legacy apps look up people and check passwords | LDAPv3 | `createLdapServer` | [LDAP directory gateway](/docs/federation/ldap) |
...
<Cards>
  ...
  <Card icon={<IdCard />} title="Verifiable credentials" href="/docs/federation/verifiable-credentials">
    SD-JWT credentials issued to digital wallets over OpenID4VCI, with status lists for revocation.
  </Card>
  <Card icon={<Radar />} title="Shared Signals receiver" href="/docs/federation/shared-signals-receiver">
    Receive CAEP and RISC events from upstream identity providers by push or poll.
  </Card>
  <Card icon={<ListTree />} title="LDAP directory gateway" href="/docs/federation/ldap">
    A read-only LDAPv3 server for VPNs, NAS devices and other apps that only speak LDAP.
  </Card>
</Cards>

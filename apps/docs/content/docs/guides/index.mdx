---
title: Introduction
description: Better IAM is an embeddable TypeScript platform for authentication, identity provisioning, and access management, running in your app on your own database.
icon: Sparkles
---

import { Blocks, Building2, Fingerprint, KeyRound, LockKeyhole, Network, ScrollText, ShieldCheck, Workflow } from '@/lib/icons';
...
<FeatureGrid>
  ...
  <Feature icon={<Workflow />} title="Governance" href="/docs/guides/governance">
    Access reviews, certification campaigns, separation of duties, role mining, invariants, change previews,
    privacy and consent, lifecycle workflows, compliance evidence, and an app catalog.
  </Feature>
  <Feature icon={<LockKeyhole />} title="Security services" href="/docs/guides/secrets-and-keys">
    Tenant keys, a secrets vault, tokenization, a private CA, SSH certificates, device posture, and identity
    threat detection.
  </Feature>
  <Feature icon={<Network />} title="Federation" href="/docs/federation">
    OAuth/OIDC sign-in and provider, SAML SSO, SCIM in and out, Shared Signals (CAEP/RISC) both ways,
    verifiable credentials, and an LDAP gateway.
  </Feature>
...
Better IAM is created and maintained by Sean Filimon. Every package is open source under the Apache License 2.0.

...
description: Give each organization its own sign-in address (a subdomain or verified custom hostname), pin its requests to it, and serve it from its home region.
icon: Globe
status: new

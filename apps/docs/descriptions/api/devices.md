...
Enrolls a public key for the caller's browser or agent, creating their device or binding the key to a device with an
enrollment code.
...
Lists the caller's devices that are not retired, each with its compliance and assurance, the device presenting this
request first.

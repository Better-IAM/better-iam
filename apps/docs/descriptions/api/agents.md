...
Returns what an agent did, newest first: the audit events of its own credentials and of the sessions in which it acted
for people.
...
Signs an agent's A2A (Agent2Agent) agent card, so other agents can check that it is a registered agent in good
